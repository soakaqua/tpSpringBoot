	...
	<div class="container">
		...
		<c:choose>
			...
			<c:when test="${personne.getClass().simpleName == 'Formateur'}">
				...
				<c:url var="action" value="/auth/personne/saveFormateur"></c:url>
			</c:when>
			<c:otherwise>
				<c:url var="action" value="/auth/personne/saveEleve"></c:url>
			</c:otherwise>
		</c:choose>
		...
		<form:form action="${action}" methode="post" modelAttribute="personne">
			...
			<div class="form-group">
				...
				<a href="${ctx}/auth/personne/list" class="btn btn-warning">annuler</a>
			</div>
